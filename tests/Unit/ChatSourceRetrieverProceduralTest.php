...
it('questionRequiresProceduralSteps returns true for strong procedural patterns', function (string $question) {
    $result = invokeRetrieverMethod(retriever(), 'questionRequiresProceduralSteps', [$question]);
    ...
    'How to renew my business license?',
    'What are the steps to file a complaint?',
    'What is the procedure for rezoning?',
]);

it('questionRequiresProceduralSteps returns false for weak procedural patterns', function (string $question) {
    $result = invokeRetrieverMethod(retriever(), 'questionRequiresProceduralSteps', [$question]);
...
it('questionAndChunkShareProceduralFocus returns true when chunk contains focus terms', function () {
    $result = invokeRetrieverMethod(retriever(), 'questionAndChunkShareProceduralFocus', [
        'how do i apply for a building permit',
        'the building department handles all residential building permits and inspections',
        'Building Department - City of Example',
    ]);

    expect($result)->toBeTrue();
});

it('questionAndChunkShareProceduralFocus returns false when chunk has no focus terms', function () {
    $result = invokeRetrieverMethod(retriever(), 'questionAndChunkShareProceduralFocus', [
        'how do i apply for a building permit',
        'the parks department manages all recreational facilities',
        'Parks Department - City of Example',
    ]);

    expect($result)->toBeFalse();
});

it('questionAndChunkShareProceduralFocus returns false for empty question', function () {
    $result = invokeRetrieverMethod(retriever(), 'questionAndChunkShareProceduralFocus', [
        '',
        'some chunk content',
        'some context',
